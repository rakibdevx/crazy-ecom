                            ...
                            <form action="{{ route('admin.admin.update', $admin->id) }}" method="POST"
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="status" class="form-control @error('status') is-invalid @enderror" required>
                                        ...
                                        <option value="suspend" {{ old('status', $admin->status) == 'suspend' ? 'selected' : '' }}>suspend</option>
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('role') is-invalid @enderror"
                                            ...
                                            multiple
                                            data-placeholder="Choose role(s)"
                                            required>
                                        @foreach ($roles as $role)
                                            <option value="{{ $role->name }}"
                                                {{ in_array($role->name, old('role', $admin->roles->pluck('name')->toArray())) ? 'selected' : '' }}>
                                                {{ $role->name }}
                                            </option>
                                        @endforeach
                                    </select>
