...
<div class="main-content">
    ...
    <div class="row">
        ...
        <div class="col-md-6 col-xl-6">
            <div class="card w-100 rounded-4">
                <div class="card-header">
                    <h5>Test Your Mail</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.setting.mail.check') }}" method="POST" enctype="multipart/form-data"
                        @class([
                            'row g-3 needs-validation',
                            'was-validated' => $errors->any()
                        ])>
                        ...
                        @csrf
                        <div class="mb-3">
                            <label class="form-label">Sent To Mail:</label>
                            <input type="email" name="sent_to_mail" value="{{ old('sent_to_mail') }}"
                                class="form-control @error('sent_to_mail') is-invalid @enderror"
                                placeholder="Recipient's email" @error('sent_to_mail') required @enderror>
                            <div class="invalid-feedback">
                                @error('sent_to_mail') {{ $message }} @enderror
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Message:</label>
                            <textarea name="sent_message" class="form-control @error('sent_message') is-invalid @enderror"
                                placeholder="Enter the message to send" @error('sent_message') required @enderror>{{ old('sent_message') }}</textarea>
                            <div class="invalid-feedback">
                                @error('sent_message') {{ $message }} @enderror
                            </div>
                        </div>

                        <div class="col">
                            <button type="submit" class="btn btn-success px-5 raised">Send</button>
                        </div>
                    </form>
                </div>
            </div>
