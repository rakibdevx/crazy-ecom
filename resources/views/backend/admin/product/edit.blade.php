                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" value="{{$product->name}}" class="form-control" name="name" placeholder="Write product title..." required>
                        </div>

                        {{-- Slug --}}
                        <div class="mb-4">
                            <h6 class="mb-2">Slug</h6>
                            <input type="text" value="{{$product->slug}}" class="form-control" name="slug" placeholder="Auto or manual slug">
                        </div>

                        {{-- Short Description --}}
                        <div class="mb-4">
                            <h5 class="mb-3">Short Description</h5>
                            <textarea class="form-control" name="short_description" rows="3" placeholder="Short summary of product...">{{$product->short_description}}</textarea>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" class="form-control d-none" id="quill-editor-area">{{$product->description}}</textarea>
                        </div>
                    </div>
                </div>
                <div class="card">
                                            ...
                                            <div class="col-12 mb-3">
                                                ...
                                                <div class="d-flex gap-3">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio" name="has_variants" id="variant_yes" value="1"  {{ $product->has_variants == '1' ? 'checked' : '' }}>
                                                        <label class="form-check-label" for="variant_yes">Yes</label>
                                                    </div>
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio" name="has_variants" id="variant_no" value="0" {{ $product->has_variants == 0 ? 'checked' : '' }}>
                                            ...
                                            <div id="simpleOptionSection" class="card mt-3">
                                                <div class="card-body">
                                                    ...
                                                    <div class="row g-3 mt-3">
                                                        <div class="col-md-6">
                                                            <h6>Cost Price</h6>
                                                            <input class="form-control" name="cost_price" type="number" value="{{$product->cost_price}}" step="0.01" placeholder="{{setting('currency_symbol')}}">
                                                        </div>
                                                        <div class="col-md-6">
                                                            <h6>Sale Price</h6>
                                                            <input class="form-control" name="sale_price" type="number" value="{{$product->sale_price}}" step="0.01" placeholder="{{setting('currency_symbol')}}">
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="row g-3 mt-3">
                                                        <div class="col-md-6">
                                                            <h6>Old Quantity</h6>
                                                            <input class="form-control" name="old_price" value="{{$product->old_price}}" type="number" placeholder="Old Price">
                                                        </div>
                                                        <div class="col-md-6">
                                                            <h6>Stock Quantity</h6>
                                                            <input class="form-control" name="stock_quantity" value="{{$product->stock_quantity}}" type="number" placeholder="Quantity">
                                                        </div>
                                                        <div class="col-md-6">
                                                            <h6>Low Stock Threshold</h6>
                                                            <input class="form-control" name="low_stock_threshold" value="{{$product->low_stock_threshold}}" type="number" placeholder="Low Stock Threshold">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="col-md-12">
                                                    ...
                                                    <div class="form-check mb-3">
                                                        <input class="form-check-input" type="radio" name="shipping_type" id="shippingProduct" value="product"
                                                            {{ $product->shipping_type == 'product' ? 'checked' : '' }}>
                                                        ...
                                                        <div id="productShippingCostInput" class="mt-2 {{ $product->shipping_type == 'product' ? '' : 'd-none' }}">
                                                            <label for="product_shipping_cost" class="form-label">Enter Product Shipping Cost</label>
                                                            <input type="number" step="0.01" min="0" class="form-control w-50" name="shipping_cost" id="product_shipping_cost" value="{{ $product->shipping_cost }}" placeholder="e.g. 120">
                                                        </div>
                                                    </div>

                                                    {{-- Flat-rate --}}
                                                    @if ($default_shipping ?? false)
                                                    <div class="form-check mb-3">
                                                        <input class="form-check-input" type="radio" name="shipping_type" id="shippingFlat"  value="flat" {{ $product->shipping_type == 'flat' ? 'checked' : '' }}>
                                                        <label class="form-check-label fw-bold" for="shippingFlat">
                                                            Flat-rate Shipping
                                                        </label>
                                                        <div class="ps-4 small text-muted">
                                                            A flat shipping cost of <strong>{{ $default_shipping->cost ?? 0 }}</strong> applies.
                                                        </div>
                                                    </div>
                                                    @endif
                                                    @if ($Zone_rate ?? false)
                                        ...
                                        <div class="tab-pane fade" id="Advanced">
                                            <div class="row g-3">
                                                <div class="col-md-6">
                                                    ...
                                                    <input class="form-control" type="text" value="{{$product->sku}}" name="sku" placeholder="SKU">
                                                </div>
                                                <div class="col-md-6">
                                                    <label class="mb-2">Barcode</label>
                                                    <input class="form-control" type="text" value="{{$product->barcode}}" name="barcode" placeholder="Barcode">
                                                </div>

                                                <div class="col-md-6">
                                                    <label>Weight (kg)</label>
                                                    <input class="form-control" type="number" value="{{$product->weight_kg}}" step="0.01" name="weight_kg">
                                                </div>
                                                <div class="col-md-6">
                                                    <label>Dimensions (L×W×H cm)</label>
                                                    <div class="d-flex gap-2">
                                                        <input class="form-control" value="{{$product->length_cm}}" type="number" step="0.01" name="length_cm" placeholder="L">
                                                        <input class="form-control" value="{{$product->width_cm}}" type="number" step="0.01" name="width_cm" placeholder="W">
                                                        <input class="form-control" value="{{$product->height_cm}}" type="number" step="0.01" name="height_cm" placeholder="H">
                                                    </div>
                                                </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                @endphp
                                <input type="text" name="tags" value="{{ implode(', ', $tags) }}" class="form-control" placeholder="Comma separated">
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        if (document.getElementById('quill-editor-area')) {
            ...
            var oldValue = @json($product->description ?? '');
            ...
            if (category_id) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        let oldSubCategory = "{{ $product->sub_category_id }}";
                        if (oldSubCategory) {
            ...
            if (subcategory_id) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        let oldCHildCategory = "{{ $product->child_category_id }}";
                        if (oldCHildCategory) {
    ...
    $(function() {
        ...
        let oldHasVariants = "{{ $product->has_variants ?? 0 }}";
        if (oldHasVariants == 1) {
